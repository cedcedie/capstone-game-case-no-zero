...
func _ready() -> void:
	...
	if CheckpointManager.has_checkpoint(CheckpointManager.CheckpointType.CELINE_CALL_COMPLETED):
		if not CheckpointManager.has_checkpoint(CheckpointManager.CheckpointType.BARANGAY_HALL_CUTSCENE_COMPLETED):
			...
			if anim_player != null:
				if anim_player.has_animation("barangay_hall_cutscene"):
					print("🎬 Playing barangay_hall_cutscene animation")
					anim_player.play("barangay_hall_cutscene")
					...
					print("⚠️ No 'barangay_hall_cutscene' animation found. Available animations: ", anim_player.get_animation_list())
