...
func _process(_delta):
	...
	if is_player_nearby and Input.is_action_just_pressed("interact") and not is_in_dialogue:
		...
		if time_since_last >= interaction_cooldown:
			...
			if not is_any_npc_in_dialogue():
				...
				print("⚠️ Station Guard: Cannot start dialogue - another NPC is already talking")
		else:
			print("⚠️ Station Guard: Interaction on cooldown - please wait")
	...
	if not is_in_dialogue and player_reference and player_reference.has_method("enable_movement"):
		...
		if player_reference.has_method("is_movement_disabled") and player_reference.is_movement_disabled():
			...
			print("🔧 Station Guard: Safety re-enabled player movement")
...
func interact():
	...
	if player_reference and player_reference.has_method("disable_movement"):
		...
		print("👮 Station Guard: Player movement DISABLED for dialogue")
...
func show_dialogue():
	...
	if player_reference and player_reference.has_method("disable_movement"):
		player_reference.disable_movement()
		print("👮 Station Guard: Player movement DISABLED for entire dialogue")
	...
	for line in dialogue_lines:
		...
		if player_reference and player_reference.has_method("disable_movement"):
			player_reference.disable_movement()
			print("👮 Station Guard: Player movement kept disabled after line")
	...
	if player_reference and player_reference.has_method("enable_movement"):
		player_reference.enable_movement()
		print("👮 Station Guard: Player movement re-enabled after dialogue")
	else:
		print("⚠️ Station Guard: Failed to re-enable player movement - player reference or method not found")
