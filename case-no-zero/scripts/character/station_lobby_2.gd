...
func _ready():
	print("🔍 station_lobby_2: _ready() called")
	...
	if interaction_area:
		...
		print("🔍 station_lobby_2: Area2D signals connected")
	else:
		print("⚠️ station_lobby_2: No Area2D found!")
...
func load_dialogue():
	var file: FileAccess = FileAccess.open("res://data/dialogues/station_lobby_2_dialogue.json", FileAccess.READ)
	if file == null:
		push_error("Cannot open station_lobby_2_dialogue.json")
		return
	...
	if typeof(parsed) != TYPE_DICTIONARY or not parsed.has("station_lobby_2"):
		push_error("Failed to parse station_lobby_2_dialogue.json")
		return
	
	dialogue_data = parsed["station_lobby_2"]
	print("✅ Loaded Station Lobby 2 dialogue")

func _on_body_entered(body):
	print("🔍 station_lobby_2: Body entered - ", body.name)
...
func show_interaction_label():
	print("🔍 station_lobby_2: Showing interaction label")
...
func show_dialogue():
	...
	print("==================================================")
	print("📋 STATION LOBBY 2 DIALOGUE:")
